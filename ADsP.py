...
#       [계층적 군집화]
            ...
            # 주요 개념
                # 계층적 방법:
                    # 병합 군집화(AGNES, Agglomerative Nesting): 처음에 각 데이터 포인트를 개별 클러스터로 보고, 유사도가 높은 클러스터를 합쳐가며 하나의 클러스터로 만듭니다.
                    # 분할 군집화(DIANA, Divisive Analysis): 처음에 모든 데이터를 하나의 클러스터로 보고, 유사성이 낮은 하위 클러스터로 분할해 나가는 방식입니다. 일반적으로 병합 군집화가 더 많이 사용됩니다.
                # 거리 측정 방법:
                ...
                # 결과 해석:
                    # 계층적 군집화는 클러스터 간의 거리를 시각적으로 나타내는 덴드로그램(Dendrogram)을 통해 해석할 수 있습니다. 덴드로그램은 데이터 포인트, 클러스터, 또는 클러스터 집단이 서로 어떻게 결합되었는지를 보여줍니다.
